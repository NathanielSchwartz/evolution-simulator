...
def gatherdata(list_of_species,list_of_food,current_tick,sim_times): # Writes data to a csv file
    ...
    with open("c:\TempPath\data_file.csv",mode="a") as data_file:
        ...
        for i in list_of_species:
            i.setall()
            data_writer.writerow([sim_times,current_tick,i.getname(),i.getpopulation(),i.getmutability(),i.getspeed(),i.getcombat(),i.getforaging()])
...
def movement(lifeform,list_of_food):
    ...
    for i in list_of_food:
        food_distance = int(round(math.sqrt(math.pow(lifeform.getx()-i.getx(),2)+math.pow(lifeform.gety()-i.gety(),2)),0))
    ...
    if closest_distance < 101:
        ...
        if closest_distance < lifeform.getspeed():
            ...
            lifeform.setenergy(int(round(lifeform.getenergy()-(food_distance/lifeform.getspeed()*math.pow(lifeform.getspeed(),1.2))/10,0)))
        else:
            # DEBUGGING
            #print("closest_distance >= lifeform.getspeed()")
            # ---------
